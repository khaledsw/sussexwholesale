...
class ModelModuleAbandonedCartReminder extends Model {
	...
	public function getCustomersForReminder() {
		$sql = "SELECT c.* FROM " . DB_PREFIX . "customer c
				WHERE TIMESTAMPDIFF(HOUR, c.date_last_action, NOW()) >= " . (int)$this->config->get('abandoned_cart_reminder_delay') . "
				      AND c.number_reminder_sent < " . (int)$this->config->get('abandoned_cart_reminder_max_reminders') ."  
				      AND c.date_last_action !='0000-00-00 00:00:00' AND c.cart != 'a:0:{}' AND c.store_id = '" . (int)$this->config->get('config_store_id') . "'
					  AND (SELECT DATE(ca.date_added) FROM " . DB_PREFIX . "customer_activity ca WHERE ca.customer_id = c.customer_id ORDER BY ca.activity_id DESC LIMIT 0,1) >= DATE('" . $this->config->get('abandoned_cart_reminder_check_start_date') . "')";
	...
	public function getGuestsForReminder() {
		$sql = "SELECT o.*, GROUP_CONCAT(DISTINCT o.order_id ORDER BY o.order_id DESC SEPARATOR ',') AS list_orders, GROUP_CONCAT(CONCAT(op.quantity, 'x', op.product_id) ORDER BY op.order_id DESC SEPARATOR ',') AS cart, IFNULL(acrg.number_reminder_sent, 0) AS number_reminder_sent, IFNULL(acrg.number_reward_sent,0) AS number_reward_sent
				FROM `order` o 
				LEFT JOIN order_product op ON (o.order_id = op.order_id)
				LEFT JOIN acr_guest acrg ON (o.order_id = acrg.order_id)
				WHERE o.customer_id = 0 AND o.order_status_id = 0 AND DATE(o.date_modified) >= DATE('" . $this->config->get('abandoned_cart_reminder_check_start_date') . "')
				AND TIMESTAMPDIFF(HOUR, o.date_modified, NOW()) >= " . (int)$this->config->get('abandoned_cart_reminder_delay') . " AND o.store_id = '" . (int)$this->config->get('config_store_id') . "'
				AND IFNULL(acrg.number_reminder_sent, 0) < " . (int)$this->config->get('abandoned_cart_reminder_max_reminders') ."
				GROUP BY o.email";
